...
const initialMessages: Message[] = [
  {
    ...
    sender: 'bot',
    // Empty string avoids SSR/client timestamp mismatch (set in useEffect after mount)
    timestamp: '',
  },
];
...
const ChatBubble: React.FC = () => {
  ...
  // Stamp the initial greeting with the current time after mount to avoid
  // SSR/client hydration mismatch from Date evaluated at different moments.
  useEffect(() => {
    ...
    setMessages((prev) =>
      prev.map((msg) =>
        msg.id === 1 && msg.timestamp === '' ? { ...msg, timestamp: now } : msg,
      ),
    );
  }, []);
      ...
      <ExpandableChatHeader>
        <div className="flex items-center space-x-3">
          <Avatar className="border-primary h-8 w-8 border-2 bg-blue-300 dark:bg-yellow-300">
            <AvatarImage src="/assets/logo.webp" alt="Assistant" />
